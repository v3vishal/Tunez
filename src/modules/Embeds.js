...
function baseEmbed(resolvable) {
  ...
  if (resolvable && typeof resolvable === "object") colour = "#00FFFF";
...
function warnMessage(interaction, text) {
  ...
  if (interaction.deferred || interaction.replied) {
    ...
    return interaction.reply({ ephemeral: false, embeds: [embedW] }).catch(console.error);
  }
}
...
function errorMessage(interaction, text) {
  ...
  if (interaction.deferred || interaction.replied) {
    ...
    return interaction.reply({ ephemeral: false, embeds: [embedE] }).catch(console.error);
  }
}
...
function queueMessage(queue, text, color) {
  ...
  const embedQ = new DJS.EmbedBuilder()
    .setDescription(text)
    .setColor(colour);
